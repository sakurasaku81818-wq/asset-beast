...
${o.log}<br>
  ...
  `,S()}function x(e,t){return t>=120&&t<=240||e>=200&&e<=360}function S(){let e=document.querySelector(`#player`),t=240,n=320;e.style.left=t+`px`,e.style.top=n+`px`,document.onkeydown=r=>{if(r.key===`e`&&s){w(s);return}let i=t,a=n;(r.key===`ArrowRight`||r.key===`d`)&&(i+=40),(r.key===`ArrowLeft`||r.key===`a`)&&(i-=40),(r.key===`ArrowDown`||r.key===`s`)&&(a+=40),(r.key===`ArrowUp`||r.key===`w`)&&(a-=40),!(i<0||i>520||a<0||a>360)&&x(i,a)&&(t=i,n=a,e.style.left=t+`px`,e.style.top=n+`px`,C(t,n))};let r=document.querySelector(`#town`);if(r){let i=0,a=0,o=!1;r.addEventListener(`touchstart`,e=>{let t=e.touches[0];i=t.clientX,a=t.clientY,o=!1},{passive:!0}),r.addEventListener(`touchmove`,e=>{let t=e.touches[0],n=t.clientX-i,r=t.clientY-a;(Math.abs(n)>15||Math.abs(r)>15)&&(o=!0,e.preventDefault())},{passive:!1}),r.addEventListener(`touchend`,r=>{let c=r.changedTouches[0];C(t,n);let l=t,u=n;if(o){let e=c.clientX-i,t=c.clientY-a;Math.abs(e)>Math.abs(t)?l+=e>0?40:-40:u+=t>0?40:-40}else{let t=e.getBoundingClientRect(),n=t.left+t.width/2,r=t.top+t.height/2,i=c.clientX-n,a=c.clientY-r;Math.abs(i)>Math.abs(a)?l+=i>0?40:-40:u+=a>0?40:-40}if(!(l<0||l>520||u<0||u>360)&&x(l,u)&&(t=l,n=u,e.style.left=t+`px`,e.style.top=n+`px`,C(t,n),!o&&s)){w(s);return}},{passive:!0})}}function C(e,t){s=null,document.querySelectorAll(`.building`).forEach(n=>{let r=n.offsetLeft,i=n.offsetTop;Math.abs(e-r)<=40&&Math.abs(t-i)<=40&&(s=n.dataset.place)});let n=document.querySelector(`.message`);n&&(s?n.innerHTML=`
    ...
    `:n.innerHTML=`
      ...
      操作：タップ・スワイプで移動
      施設の近くでタップして入る
    `)}function w(e){e===`securities`&&T(),e===`realestate`&&O(),e===`home`&&L(),e===`work`&&A(),e===`school`&&I()}window.enterPlace=w;function T(){a.innerHTML=`
...
${t.place}
💰 最低日給 +3,000円
⭐ 評価は上がらなかった`)),A()}document.querySelector(`#workA`).onclick=()=>r(n[0]),document.querySelector(`#workB`).onclick=()=>r(n[1]),document.querySelector(`#workC`).onclick=()=>r(n[2]),document.querySelector(`#backWork`).onclick=A}function I(){a.innerHTML=`
    <div class="screen">
      <h2>🎓 学校</h2>
  ...
  `,document.querySelector(`#studyFP`).onclick=z,document.querySelector(`#backTown`).onclick=b}function L(){a.innerHTML=`
    <div class="screen">
      <h2>🏡 自宅</h2>
      <p>家で休むと1ヶ月が進みます。</p>
