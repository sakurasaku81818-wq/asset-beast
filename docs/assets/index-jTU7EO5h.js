...
${o.log}<br>
  ...
  `,x()}function b(e,t){return t>=120&&t<=240||e>=200&&e<=360}function x(){let e=document.querySelector(`#player`),t=240,n=320;e.style.left=t+`px`,e.style.top=n+`px`,document.onkeydown=r=>{if(r.key===`e`&&s){C(s);return}let i=t,a=n;(r.key===`ArrowRight`||r.key===`d`)&&(i+=40),(r.key===`ArrowLeft`||r.key===`a`)&&(i-=40),(r.key===`ArrowDown`||r.key===`s`)&&(a+=40),(r.key===`ArrowUp`||r.key===`w`)&&(a-=40),!(i<0||i>520||a<0||a>360)&&b(i,a)&&(t=i,n=a,e.style.left=t+`px`,e.style.top=n+`px`,S(t,n))};let r=document.querySelector(`#town`);if(r){let i=0,a=0,o=!1;r.addEventListener(`touchstart`,e=>{let t=e.touches[0];i=t.clientX,a=t.clientY,o=!1},{passive:!0}),r.addEventListener(`touchmove`,e=>{let t=e.touches[0],n=t.clientX-i,r=t.clientY-a;(Math.abs(n)>15||Math.abs(r)>15)&&(o=!0,e.preventDefault())},{passive:!1}),r.addEventListener(`touchend`,r=>{let c=r.changedTouches[0];S(t,n);let l=t,u=n;if(o){let e=c.clientX-i,t=c.clientY-a;Math.abs(e)>Math.abs(t)?l+=e>0?40:-40:u+=t>0?40:-40}else{let t=e.getBoundingClientRect(),n=t.left+t.width/2,r=t.top+t.height/2,i=c.clientX-n,a=c.clientY-r;Math.abs(i)>Math.abs(a)?l+=i>0?40:-40:u+=a>0?40:-40}if(!(l<0||l>520||u<0||u>360)&&b(l,u)&&(t=l,n=u,e.style.left=t+`px`,e.style.top=n+`px`,S(t,n),!o&&s)){C(s);return}},{passive:!0})}}function S(e,t){s=null,document.querySelectorAll(`.building`).forEach(n=>{let r=n.offsetLeft,i=n.offsetTop;Math.abs(e-r)<=40&&Math.abs(t-i)<=40&&(s=n.dataset.place)});let n=document.querySelector(`.message`);n&&(s?n.innerHTML=`
    ...
    `:n.innerHTML=`
      ...
      操作：タップ・スワイプで移動
      施設の近くでタップして入る
    `)}function C(e){e===`securities`&&w(),e===`realestate`&&D(),e===`home`&&j(),e===`work`&&k(),e===`school`&&A()}window.enterPlace=C;function w(){a.innerHTML=`
...
<div class="card">
  ...
  `,document.querySelector(`#backTown`).onclick=y,document.querySelector(`#buyHouse`)&&(document.querySelector(`#buyHouse`).onclick=()=>O(`戸建て`,3e7)),document.querySelector(`#buyMansion`)&&(document.querySelector(`#buyMansion`).onclick=()=>O(`マンション`,45e6)),document.querySelector(`#buyApartment`)&&(document.querySelector(`#buyApartment`).onclick=()=>O(`アパート`,8e7))}function O(e,t){if(o.cash<t){alert(`資金が足りません`);return}o.cash-=t,o.house={name:e,purchasePrice:t,cost:0},o.propertyValue=t,o.log=`${e}を${d(t)}で購入しました。`,y()}function k(){a.innerHTML=`
    <div class="screen">
      <h2>🏢 仕事</h2>
      <p>${o.job.name}として働いている。</p>
      <p>月収：${d(o.job.income*o.region.salaryRate)}</p>
      <button id="backTown">街へ戻る</button>
    </div>
  `,document.querySelector(`#backTown`).onclick=y}function A(){a.innerHTML=`
    <div class="screen">
      <h2>🎓 学校</h2>
  ...
  `,document.querySelector(`#studyFP`).onclick=N,document.querySelector(`#backTown`).onclick=y}function j(){a.innerHTML=`
    <div class="screen">
      <h2>🏡 自宅</h2>
      <p>家で休むと1ヶ月が進みます。</p>
...
${t.explanation}`),A()}}),document.querySelector(`#backSchool`).onclick=A}function P(e,t){return Math.random()*(t-e)+e}function F(){let e=[{text:`🤖 AI投資ブーム。関連企業に資金が流入。`,stock:.06,etf:.02,reit:0,crypto:.03},{text:`💾 半導体需要が急増。関連株が上昇。`,stock:.05,etf:.03,reit:0,crypto:.01},{text:`💱 急激な円安で輸入物価が上昇。`,stock:-.02,etf:-.01,reit:-.01,crypto:.02},{text:`💴 円高が進み、輸入コストが低下。`,stock:.02,etf:.01,reit:.01,crypto:-.01},{text:`🛢 原油価格が急騰。企業コストが上昇。`,stock:-.04,etf:-.02,reit:-.02,crypto:-.01},{text:`☀️ 太陽光発電への大型投資が発表された。`,stock:.03,etf:.02,reit:.01,crypto:0},{text:`⚛️ 原子力発電所の再稼働が決定。`,stock:.03,etf:.01,reit:0,crypto:0},{text:`⚡ エネルギー価格が大幅に低下。`,stock:.04,etf:.03,reit:.02,crypto:.01},{text:`🌾 小麦の不作で食料価格が上昇。`,stock:-.02,etf:-.01,reit:0,crypto:0},{text:`🍚 米が豊作。食品価格が安定した。`,stock:.01,etf:.01,reit:0,crypto:0},{text:`🐟 記録的な豊漁。食品関連企業に追い風。`,stock:.02,etf:.01,reit:0,crypto:0},{text:`🚀 民間ロケットの打ち上げに成功。`,stock:.04,etf:.02,reit:0,crypto:.02},{text:`🛰 次世代衛星通信サービスが開始された。`,stock:.04,etf:.02,reit:0,crypto:.01},{text:`🌪 大型台風が都市部を直撃。`,stock:-.04,etf:-.03,reit:-.06,crypto:-.01},{text:`🌊 大規模洪水が発生。インフラ被害が拡大。`,stock:-.05,etf:-.03,reit:-.07,crypto:-.02},{text:`❄️ 記録的寒波で物流が混乱。`,stock:-.03,etf:-.02,reit:-.01,crypto:0},{text:`🏗 巨額のインフラ投資計画が発表された。`,stock:.05,etf:.03,reit:.04,crypto:0},{text:`🚄 新しい高速鉄道計画が始動。`,stock:.03,etf:.02,reit:.03,crypto:0},{text:`🎬 世界的大ヒット映画が誕生。`,stock:.02,etf:.01,reit:0,crypto:.01},{text:`📺 動画配信サービスの利用者が急増。`,stock:.03,etf:.02,reit:0,crypto:.01},{text:`🗳 選挙結果を受け市場に不透明感が広がる。`,stock:-.03,etf:-.02,reit:-.01,crypto:.01},{text:`🏛 新政権が大型経済対策を発表。`,stock:.05,etf:.03,reit:.02,crypto:.02},{text:`⚔️ 地域紛争が激化。世界市場が不安定に。`,stock:-.06,etf:-.04,reit:-.03,crypto:-.05},{text:`🕊 停戦合意が成立。市場に安心感が広がる。`,stock:.05,etf:.03,reit:.02,crypto:.03},{text:`₿ 仮想通貨の税制改革が発表された。`,stock:.01,etf:.01,reit:0,crypto:.1},{text:`🚫 仮想通貨規制が強化された。`,stock:0,etf:0,reit:0,crypto:-.12},{text:`🌍 世界景気が回復。市場全体が上昇。`,stock:.05,etf:.03,reit:.02,crypto:.04},{text:`😐 大きなニュースはなかった。`,stock:0,etf:0,reit:0,crypto:0}];return e[Math.floor(Math.random()*e.length)]}function I(){let e=[{text:`昇進した。月収が1万円上がった。`,cash:0,income:1e4},{text:`ボーナス20万円を受け取った。`,cash:2e5,income:0},{text:`医療費5万円を支払った。`,cash:-5e4,income:0},{text:`副業収入3万円が入った。`,cash:3e4,income:0},{text:`特に何も起こらなかった。`,cash:0,income:0}];return e[Math.floor(Math.random()*e.length)]}function L(){let e=`時間を味方につけた者が、最後に勝つ。.複利は世界で最も強力な力の一つ。.卵は一つのカゴに盛るな。.安い時に恐れず、高い時に浮かれない。.投資はマラソン。短距離走ではない。.暴落は優良資産のバーゲンセール。.現金は守り、投資は攻め。.感情は投資の最大の敵。.利益は我慢した人に訪れる。.最悪なのは何もしないこと。.市場は短期では投票機、長期では計量機。.未来は誰にも読めない。だから分散する。.毎月積み立てることが最大の武器。.価格ではなく価値を見極めよう。.リスクとは値動きではなく、理解不足である。.お金にも働いてもらおう。.収入より支出の管理が重要。.資産形成は生活習慣で決まる。.今日の浪費は未来の資産を減らす。.焦って売買すると手数料だけが増える。.暴騰より継続が強い。.人生最大の投資は自分への投資。.知識は最高の資産。.借金は使い方次第で武器にもなる。.配当は小さな給料日。.長期投資は時間との共同作業。.チャンスは悲観の中に生まれる。.相場を当てるより、生き残ることが大切。.市場から退場しない人が勝者になる。.資産家はお金のためではなく、自由のために投資する。`.split(`.`);return e[Math.floor(Math.random()*e.length)]}function R(){a.innerHTML=`
    <div class="screen title-screen">
      <h1>結果発表</h1>
      <p>あなたの総資産</p>
      <h2>${d(f())}</h2>
